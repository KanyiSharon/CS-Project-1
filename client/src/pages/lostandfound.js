...
const LostAndFound = () => {
  ...
  const [form, setForm] = useState({ lostitem: '', sacco_name: '', date: '', sacco:'', description: '', image: null });
      ...
      <div className="auth-box">
        ...
        {user?.role === 'Driver' && (
          <>
            ...
            <form onSubmit={handleSubmit} className="post-form">
              ...
              <input type="text" name="sacco_name" placeholder="Sacco Name (e.g. CBD to Westlands)" onChange={handleChange} required />
        ...
        {items.map((item, idx) => (
          <div key={idx} style={{ border: '1px solid #ccc', padding: '10px', marginBottom: '10px', borderRadius: '8px' }}>
            ...
            <strong>Sacco Name:</strong> {item.sacco_name}<br />
...
// Note: The above code assumes you have a backend API set up to handle the lost and found items.
// The API endpoints used (e.g., `/api/lost-items`, `/api/lost-item`) should be implemented in your backend server.
// The `claim` route should also be implemented to handle the claiming of found items.